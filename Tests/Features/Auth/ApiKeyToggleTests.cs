...
public class ApiKeyToggleTests : TestBase
{
    ...
    public async Task ShouldToggleApiKeyFromActiveToInactive()
    {
        ...
        var createResponse = await Client.PostAsJsonAsync("/api/auth/apikey", new { Name = "Toggle Me" });
        ...
        var toggleResponse = await Client.PutAsJsonAsync("/api/auth/apikey/toggle", togglePayload);
    ...
    public async Task ShouldToggleApiKeyFromInactiveToActive()
    {
        ...
        var createResponse = await Client.PostAsJsonAsync("/api/auth/apikey", new { Name = "Toggle Me 2" });
        var apiKey = await createResponse.Content.ReadFromJsonAsync<CreateApiKeyResponse>();
        apiKey.Should().NotBeNull();
        ...
        var toggleBackResponse = await Client.PutAsJsonAsync("/api/auth/apikey/toggle", toggleBackPayload);
    ...
    public async Task ShouldReturn404WhenTogglingApiKeyBelongsToAnotherUser()
    {
        ...
        var createResponse = await Client.PostAsJsonAsync("/api/auth/apikey", new { Name = "Owner Key" });
        var apiKey = await createResponse.Content.ReadFromJsonAsync<CreateApiKeyResponse>();
    ...
    public async Task ShouldNotAllowApiKeyToWorkWhenInactive()
    {
        ...
        var createResponse = await Client.PostAsJsonAsync("/api/auth/apikey", new { Name = "Disable Me" });
        var apiKey = await createResponse.Content.ReadFromJsonAsync<CreateApiKeyResponse>();
        apiKey.Should().NotBeNull();
        ...
        var validateResponse = await Client.PostAsJsonAsync("/api/licenses/validate", new
        {

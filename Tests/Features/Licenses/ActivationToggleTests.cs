...
public class ActivationToggleTests : TestBase
{
    ...
    public async Task ShouldToggleActivationFromActiveToInactive()
    {
        ...
        var toggleResponse = await Client.PutAsJsonAsync($"{ApiVersion.Path}/licenses/activations/toggle", togglePayload);
    ...
    private async Task<(Guid licenseId, string token)> CreateLicenseAndGetId(string email)
    {
        ...
        var licenseResponse = await Client.PostAsJsonAsync($"{ApiVersion.Path}/licenses", new { ProductId = product!.Id, Name = "License", MaxActivations = 5 });
